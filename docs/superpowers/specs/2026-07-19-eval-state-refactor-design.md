...
同步删除四个字段。DTO 层 `internal/applications/dtos/eval.go:107-112` 里的四行 `QueuedAt / HeartbeatAt / DeadlineAt / WorkerID` 全部删除。注意 `WorkerID` 现有定义 `WorkerID string json:"worker_id"` **没有** `omitempty`，删字段（而非仅指针化）是唯一正确做法。`ErrorMessage` 语义从"总为空"变为"终态时非空（PASSED 除外）"。

**前端契约变更清单**（供 mooc-manus-web 联动 PR 参考）：
- 响应 JSON 里不再有 `queued_at / heartbeat_at / deadline_at / worker_id` 4 个 key
- `error_message` 在终态时非空字符串，PASSED / PENDING / RUNNING 时为空
...
InstanceExecutor.Execute(ctx, instanceID)          ← 单函数
  ...
  │       cerr != nil          → target=FAILED;  errMsg="chat_stage: runner_err: <err>"
  │       chatRes.DidTimeout   → target=TIMEOUT; errMsg="chat_stage: chat_timeout=<X>s"
...
**detail 组装规则**（chat_stage 三个子分支的 detail 前缀写死为固定字符串，方便 §5.3 E2E 断言精准匹配）：
...
chat_stage        : 分三个子分支，detail 前缀锁死：
  - chatRunner 返回 err       → "runner_err: <err.Error()>"           → target=FAILED
  - chatRes.DidTimeout == true → "chat_timeout=<X>s"                   → target=TIMEOUT
  - chatRes.Error != nil       → "agent_error: <chatRes.Error.Error()>" → target=FAILED
...
评审后补齐至 **11 个文件**（先前遗漏了 `eval_run_instance_test.go` 与 `applications/services/eval_test.go`）。**已确认引用被删符号/字段的完整文件清单**（由 `grep -rln "queued_at|heartbeat_at|deadline_at|worker_id|UpdateHeartbeat|..." internal api` 生成）：

| 文件 | 命运 | 说明 |
|-----|-----|------|
| `internal/domains/services/evaluation/state_machine_test.go` | **重写** | 白名单从 7 转换点缩到 6 转换点；删所有 QUEUED/INITIALIZING/VERIFYING/CANCELED 用例 |
| `internal/domains/services/evaluation/executor_cas_test.go` | **改写** | 只测 `PENDING→RUNNING` 幂等（多 worker 竞速一个赢）；删中间态 CAS 用例 |
| `internal/domains/services/evaluation/executor_init_test.go` | **改写** | 测 mkdir_workspace / init_script 失败路径 → error_message reason 命中 |
| `internal/domains/services/evaluation/executor_chat_verify_test.go` | **改写** | chat_stage 三个子分支 `runner_err:` / `chat_timeout=` / `agent_error:` 前缀断言 |
| `internal/domains/services/evaluation/executor_finalize_test.go` | **重写** | **删除** `TestExecutor_FinalizeVerify_HeartbeatFires`（心跳依赖用例）；新增单点 finalize（PASSED/FAILED/TIMEOUT 三分支），断言 `result.Upsert` 与 `FinalizeRunning` 恰好各调一次 |
| `internal/domains/services/evaluation/executor_stubs_test.go` | **调整** | `stubInstRepo` 删 `UpdateHeartbeat/UpdateQueuedAt/ListStaleInstances`；新增 `FinalizeRunning/ListStaleRunning` |
| `internal/domains/services/evaluation/service_impl_test.go` | **调整** | `fakeInstRepo` 同步接口变化；`TestSweepStaleInstances_*` 改成 started_at 判据 |
| `internal/domains/services/evaluation/e2e_test.go` | **改写** | 2×2 生命周期只期望 PENDING→RUNNING→终态；**e2e_test.go:119 case 里的 `InstanceStatusCanceled` 分支删除**；删心跳/deadline 子用例 |
| `internal/domains/services/evaluation/stress_test.go` | **调整** | 并发压测断言口径改为"每个 instance ≤ 2 次 UPDATE + 1 次 result Upsert" |
| **`internal/infra/repositories/eval_run_instance_test.go`** | **重写** | 建表/断言里的 `queued_at/heartbeat_at/deadline_at/worker_id` 列全部删；新增 `FinalizeRunning` / `ListStaleRunning` 的直接 SQL 层用例 |
| **`internal/applications/services/eval_test.go`** | **调整** | 涉及 `InstanceView.WorkerID/QueuedAt/HeartbeatAt/DeadlineAt` 的断言全部删；`enqueueInstances` 相关用例删除 `UpdateQueuedAt` 期望 |
...
- **`TestNoCanceledLingering`**：静态检查用例，`grep -rn 'InstanceStatusCanceled\|"CANCELED"' internal api` 期望 0 命中，防未来回潮
...
| 用例 | 前置 | 动作 | 期望 |
|-----|-----|------|-----|
| 2.1 agent 超时 | chatTimeout=5s，prompt 触发 60s 等待 | 提交任务 | instance→**TIMEOUT**；`error_message` 首行 `chat_stage: chat_timeout=5s` |
| 2.2 agent 返回 ErrorEvent | 错误 apiKey | 提交任务 | instance→FAILED；`error_message` 首行 `chat_stage: agent_error: <上游 err>` |
| 2.3 chatRunner 内部 err | snapshot 缺 model 字段 | 提交任务 | instance→FAILED；`error_message` 首行 `chat_stage: runner_err: <err>` |
...
| 步 | 名称 | 主要文件 | 交付物 |
|----|-----|--------|--------|
| 1 | 状态枚举收缩 | `internal/domains/models/evaluation/status.go` | 删 `Queued/Initializing/Verifying/Canceled` 常量；**同步修 `IsTerminal()`（status.go:27 移除 `InstanceStatusCanceled` 分支）**；**同步删 executor.go:300 心跳 goroutine 里读到 CANCELED 判 cancel 的逻辑**（该行会在 §6.1 步骤 5 心跳整体移除时消失，但本步先把 CANCELED 引用摘掉即可编译）；**同步删 e2e_test.go:119 switch 里的 `InstanceStatusCanceled` case**；**核查 `internal/infra/repositories/eval_run_instance.go:119` 里 SQL 白名单 `NOT IN ('PASSED','FAILED','TIMEOUT','CANCELED')` → 改为 `NOT IN ('PASSED','FAILED','TIMEOUT')`** |
...
| 6 | Application/Domain 层同步 | `service_impl.go`、`applications/services/eval.go` | `SweepStaleInstances` (service_impl.go:427) 改用 `ListStaleRunning + FinalizeRunning`；**`RetryInstance` (service_impl.go:290,293,380,383) 清 4 字段的代码全部删除，独立列出**；**`ArchiveDeadTasks` (service_impl.go:510)** 若持有旧状态 CAS from 需校核并适配 5 状态集；`instanceDOToView` (applications/services/eval.go:520) DTO 同步；`enqueueInstances` (applications/services/eval.go:439) 删 `UpdateQueuedAt` 调用 |
| 7 | route.go 装配调整 | `api/routers/route.go`、`config/config.go`、**`config/config.toml`** | `NewInstanceExecutor` 少传 `heartbeatInterval`；`EvaluationConfig.HeartbeatIntervalSec` 删除 + `SweeperStaleGraceSec` 新增（默认 60s）；**`config/config.toml` 里 `heartbeat_interval_sec` 键删除、`sweeper_stale_grace_sec` 键新增** |
| 8 | 单测重写/调整 | 见 §5.1 | 11 个 test 文件按"重写/调整"列执行（见 §5.1 完整清单）；新增 6 个用例（§5.2） |
...
**部署顺序硬约束（发布链）**：**先发布新代码，再跑 migration**。原因：
- 若"先 migration 后代码"：DB 已删列，旧代码里的 PO 仍带 `heartbeat_at` 字段 → GORM `SELECT * FROM eval_run_instance` 会报 `column "heartbeat_at" does not exist`。
- 若"先代码后 migration"：DB 还有旧列，新 PO 已不含字段 → GORM SELECT 只取新字段，旧列被 orphan 但服务功能正常。旧列在 migration 后被 drop，无影响。

因此正确顺序：**步骤 1-10 全部合并部署 → 灰度验证服务正常 → 单独执行步骤 4 的 migration**。步骤 4 在 §6.1 表格里排位靠前是为了让 spec 读者理解 schema 变化，实际执行顺序上 migration 是**最后一步**。
...
| **滚动升级窗口 asynq Retention(72h)**：旧 worker 仍执行旧代码，`UpdateHeartbeat` / `UpdateQueuedAt` 会因列缺失全部报错，导致大量幂等重试与 EVAL_MQ_* 告警噪声 | 高 | 严格遵循"先代码后 migration"部署顺序（见 §6.1 部署顺序硬约束）；灰度期间监控 `EVAL_MQ_INSTANCE_MISS / EVAL_STAGE_FINALIZE_CAS_MISS` 日志级别；如果发现旧 worker 未清理，紧急 `asynq queue purge` |
